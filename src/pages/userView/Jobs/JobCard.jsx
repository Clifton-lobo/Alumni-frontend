...
import {
  MapPin, Briefcase, Clock, IndianRupee, Building2,
...
const JobCard = ({ job, view = "grid", onApply }) => {
  ...
  const handleApply = () => {
    if (!isAuthenticated) {
      setLoginPromptMessage("Sign in to apply for this job");
  ...
  const handlePosterClick = () => {
    if (!isAuthenticated) {
      setLoginPromptMessage("Sign in to view alumni profiles");
      setShowLoginPrompt(true);
      return;
    }
  ...
  const renderApplyButton = () => (
    <button
      onClick={handleApply}
      className="group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl hover:bg-[#1e2d4d] text-black hover:text-white transition-all duration-200 cursor-pointer"
    >
      Apply
      {isExternal ? (
    ...
    >
      ...
      <div className="w-7 h-7 rounded-full overflow-hidden bg-[#142A5D] flex items-center justify-center flex-shrink-0 ring-2 ring-white shadow-sm group-hover/poster:ring-[#EBAB09] transition-all">
        {job.postedBy?.profilePicture ? (
          <img src={job.postedBy.profilePicture} alt={getDisplayName()} className="w-full h-full object-cover" />
        ) : (
          <span className="text-white text-[11px] font-bold">
            {isAdminPosted
              ? "A"
              : (job.postedBy?.fullname || job.postedBy?.username || "?").charAt(0).toUpperCase()}
          </span>
        )}
      </div>
      <span className={`text-sm font-semibold group-hover/poster:underline underline-offset-2 transition-colors ${isAdminPosted ? "text-green-700" : "text-gray-900 group-hover/poster:text-[#142A5D]"}`}>
        {getDisplayName()}
        {isAdminPosted && <Shield className="inline w-3 h-3 text-green-600 ml-1 -mt-0.5" />}
      </span>
    </button>
  );
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  ...
                  {job.experienceLevel && (
                    <span className="px-3 py-1 text-xs rounded-full border border-gray-200 bg-white text-gray-700">
                      {job.experienceLevel === "fresher" ? "fresher exp" : `${job.experienceLevel} exp`}
                    </span>
                  )}
                  {job.openings > 1 && (
                ...
                <div className="flex flex-wrap items-center gap-8 mt-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {job.salary?.disclosed ? (
                      <span className="font-medium text-gray-800">₹{job.salary.min?.toLocaleString()} – ₹{job.salary.max?.toLocaleString()}</span>
                    ) : (
