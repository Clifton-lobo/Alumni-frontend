  ...
  import {
    Images, Upload, CheckCircle, XCircle, Clock, Trash2, Edit2,
    Loader2, AlertCircle, Plus, Search, X, Calendar, Merge,
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${s.color}`}>
  ...
  const Lightbox = ({ photos, startIndex, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center" style={{ background: "rgba(0,0,0,0.96)" }}>
        <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition">
          <X className="h-5 w-5 text-white" />
        </button>
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-white/40">{idx + 1} / {photos.length}</div>
        {idx > 0 && (
          <button onClick={() => setIdx((i) => i - 1)} className="absolute left-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition">
        ...
        {idx < photos.length - 1 && (
          <button onClick={() => setIdx((i) => i + 1)} className="absolute right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition">
  ...
  /* ══════════════════════════════════════════════
    ALBUM PHOTO VIEWER DIALOG (admin)
  ══════════════════════════════════════════════ */
  const AlbumPhotosDialog = ({ album, open, onClose, onRefresh }) => {
    ...
    const load = useCallback(async () => {
      ...
      setLoading(true);
      try {
        const res = await axios.get(`/api/admin/gallery/albums/${album._id}/photos`, { withCredentials: true });
        setPhotos(res.data.data);
      } catch { toast.error("Failed to load photos"); }
      finally { setLoading(false); }
    }, [album]);

    useEffect(() => { if (open && album) load(); }, [open, album]);
    ...
    const handleAddPhotos = async (e) => {
      ...
      try {
        await axios.post(`/api/admin/gallery/albums/${album._id}/photos`, fd, {
    ...
    const handleDelete = async (photoId) => {
      try {
        await axios.delete(`/api/admin/gallery/photos/${photoId}`, { withCredentials: true });
    ...
    const handleSetCover = async (photoId) => {
      try {
        await axios.patch(`/api/admin/gallery/albums/${album._id}/set-cover/${photoId}`, {}, { withCredentials: true });
            ...
            <DialogHeader className="flex-shrink-0 px-6 py-5 border-b border-gray-100">
              <div className="flex items-center justify-between pr-8">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">{photos.length} photos</p>
                </div>
                <button
                ...
                <input ref={fileRef} type="file" multiple accept="image/*" className="hidden" onChange={handleAddPhotos} />
              </div>
            </DialogHeader>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {loading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {photos.map((photo, i) => (
                    <div key={photo._id} className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 cursor-pointer"
  ...
  /* ══════════════════════════════════════════════
    REJECT DIALOG
  ══════════════════════════════════════════════ */
  const RejectDialog = ({ album, open, onClose, onRejected }) => {
    ...
    const handle = async () => {
      setLoading(true);
      try {
        await axios.patch(`/api/admin/gallery/albums/${album._id}/reject`, { reason }, { withCredentials: true });
        toast.success("Album rejected");
        onRejected(); onClose();
      } catch { toast.error("Failed to reject album"); }
      finally { setLoading(false); }
    };
    ...
    return (
      <Dialog open={open} onOpenChange={(v) => { if (!v) { onClose(); setReason(""); } }}>
        <DialogContent className="max-w-sm rounded-2xl border-0">
          ...
          <p className="text-sm text-gray-500 -mt-2">Rejecting: <strong className="text-gray-700">{album?.title}</strong></p>
          <textarea
            value={reason}
  ...
  const MergeDialog = ({ selected, allAlbums, open, onClose, onMerged }) => {
    ...
    const handle = async () => {
      ...
      setLoading(true);
      try {
        await axios.patch("/api/admin/gallery/albums/merge",
          { albumIds: selected, title: title.trim() },
          { withCredentials: true }
        );
        toast.success(`${selected.length} albums merged successfully`);
        onMerged(); onClose(); setTitle("");
            ...
            {selectedAlbums.map((a) => (
              <div key={a._id} className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50">
                <div className="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 bg-gray-200">
                  {a.coverImage?.url && <img src={a.coverImage.url} alt="" className="w-full h-full object-cover" />}
                </div>
  ...
  /* ══════════════════════════════════════════════
    UPLOAD SHEET (Admin creates album)
  ══════════════════════════════════════════════ */
  const UploadSheet = ({ open, onClose, onCreated }) => {
    ...
    const handle = async () => {
      ...
      files.forEach((f) => fd.append("photos", f));
      try {
        await axios.post("/api/admin/gallery/albums", fd, {
          withCredentials: true, headers: { "Content-Type": "multipart/form-data" },
        });
      ...
      <Sheet open={open} onOpenChange={(v) => { if (!v) { onClose(); reset(); } }}>
        <SheetContent side="right" className="w-full sm:max-w-[520px] p-0 flex flex-col gap-0">
          ...
          <div className="flex-shrink-0 px-6 py-5 border-b border-gray-100"
            ...
            <p className="text-white/40 text-xs mt-1">Admin uploads are auto-approved and go live immediately.</p>
          </div>
            ...
            <div>
              ...
              <div
                className={`rounded-2xl border-2 p-6 text-center cursor-pointer transition-all
                  ${dragging ? "border-[#EBAB09] bg-amber-50" : "border-dashed border-gray-200 hover:border-gray-300"}`}
              ...
              >
                ...
                <p className="text-sm text-gray-500">Drop photos or <span className="font-semibold" style={{ color: GOLD }}>click to browse</span></p>
                <p className="text-xs text-gray-400 mt-1">Up to 50 images · First image becomes cover</p>
              </div>

              {previews.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs text-gray-400 mb-2">{files.length} photo{files.length !== 1 ? "s" : ""} selected</p>
          ...
          <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3">
            <button onClick={() => { onClose(); reset(); }} className="px-5 py-2.5 rounded-xl text-sm font-medium text-gray-500 hover:text-gray-700">
              Cancel
            </button>
            <button onClick={handle} disabled={loading}
              ...
              style={{ background: NAVY }}>
              {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating…</> : <><Plus className="h-4 w-4" /> Create Album</>}
            </button>
          </div>
        </SheetContent>
      </Sheet>
  ...
  const Gallery = () => {
    ...
    const load = useCallback(async (page = 1) => {
      ...
      try {
        ...
        const res = await axios.get(`/api/admin/gallery/albums?${params}`, { withCredentials: true });
    ...
    const handleApprove = async (album) => {
      setApproving(album._id);
      try {
        await axios.patch(`/api/admin/gallery/albums/${album._id}/approve`, {}, { withCredentials: true });
    ...
    const handleDelete = async (album) => {
      ...
      try {
        await axios.delete(`/api/admin/gallery/albums/${album._id}`, { withCredentials: true });
    ...
    return (
      <div className="min-h-screen" style={{ background: "#F7F6F3", fontFamily: "'Outfit', system-ui, sans-serif" }}>
            ...
            <div>
              ...
              {pendingCount > 0 && (
                <p className="text-xs text-amber-600 font-medium mt-0.5 flex items-center gap-1">
                  <Clock className="h-3 w-3" /> {pendingCount} album{pendingCount !== 1 ? "s" : ""} awaiting review
                </p>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {selected.length > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs font-medium text-amber-700">
                ...
                <button onClick={() => setSelected([])} className="hover:text-amber-900">
                  <X className="h-3 w-3" />
                </button>
              </div>
            )}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-sm text-gray-400">
                {statusFilter === "pending" ? "All user submissions have been reviewed." : "Create your first album to get started."}
              </p>
            </div>
          ) : (
            <>
      ...
      ${isSelected
                    ...
                    >
                      ...
                      <div className="absolute top-3 left-3 z-10">
                        <button
                          ...
                          className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all
                            ${isSelected ? "border-[#EBAB09] bg-[#EBAB09]" : "border-white/60 bg-black/20 opacity-0 group-hover:opacity-100"}`}>
                      ...
                      {/* Info */}
                      <div className="p-4">
                        <h3 className="text-sm font-bold text-gray-900 line-clamp-1 mb-0.5">{album.title}</h3>
              ...
              {pagination.pages > 1 && (
                <div className="flex items-center justify-center gap-2 pt-4">
                  ...
                  <span className="text-sm text-gray-500 px-3">{pagination.page} / {pagination.pages}</span>
        ...
        {/* Overlays */}
        <UploadSheet open={uploadOpen} onClose={() => setUploadOpen(false)} onCreated={() => load(1)} />
        <MergeDialog selected={selected} allAlbums={albums} open={mergeOpen}
          onClose={() => setMergeOpen(false)} onMerged={() => load(1)} />
        <RejectDialog album={rejectTarget} open={!!rejectTarget}
          onClose={() => setRejectTarget(null)} onRejected={() => load(pagination.page)} />
        <AlbumPhotosDialog album={viewAlbum} open={!!viewAlbum}
          onClose={() => setViewAlbum(null)} onRefresh={() => load(pagination.page)} />
      </div>
    );
  };
